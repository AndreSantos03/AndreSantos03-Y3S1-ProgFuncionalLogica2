...
data StackVal = IVal Integer | BVal Bool deriving Show
type Stack = [StackVal]
...
stack2Str :: Stack -> String
stack2Str stack =
    trace ("Stack: " ++ formattedStack) formattedStack
  where
    formattedStack = intercalate "," . reverse . map showStackVal $ stack

state2Str :: State -> String
state2Str (_, store) =
    trace ("State: " ++ formattedState) formattedState
  where
    formattedState = intercalate "," . map showVarVal . sortBy (comparing fst) $ store
    showVarVal :: (String, StackVal) -> String
    showVarVal (var, IVal n) = var ++ "=" ++ show n
    showVarVal (var, BVal b) = var ++ "=" ++ map toLower (show b)
